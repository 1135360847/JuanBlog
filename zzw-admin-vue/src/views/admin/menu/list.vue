<template>
  <div class="app-container">
    ...
    <el-form :inline="true" class="demo-form-inline">
      ...
      <el-button type="primary" icon="el-icon-search" @click="fetchNodeList()">查询</el-button>
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="text"
            size="mini"
            @click="() => {restData(),dialogFormVisible = true, menu.pid = scope.row.id}">添加菜单
          </el-button>
          <el-button
            v-if="scope.row.level == 3 &&  hasPerm('permission.add')"
            type="text"
            size="mini"
            @click="() => {restData(),dialogPermissionVisible = true, permission.pid = scope.row.id}">添加功能
          </el-button>
          <el-button
          ...
          <el-button
            v-if="scope.row.level != 4 && scope.row.name != '全部数据' && hasPerm('permission.update')"
            type="text"
            size="mini"
          ...
          <el-button
            type="text"
            size="mini"
            @click="() => remove(scope.row)" v-if="hasPerm('permission.remove') && scope.row.name != '全部数据'">删除
          </el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog v-dialogDrag :visible.sync="dialogFormVisible" :title="dialogFormValue">
    ...
    <!-- 添加功能的窗口 -->
    <el-dialog v-dialogDrag :visible.sync="dialogPermissionVisible" title="添加功能">
...
export default {
  ...
  methods: {
    ...
    fetchNodeList() {
      menu.getNestedTreeList(this.searchObj).then(response => {
